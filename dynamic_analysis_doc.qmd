...
Load impact dataset, use nuts-3 level (districts) for analysis
...
visualize_impacts_across_years <- impacts_dataset %>% 
  dplyr::filter(statistical_unit  == "nuts3") %>% 
...
expand.grid(type_of_class = unique(impacts_dataset$type_of_class),
            year_date = seq(2000,2022,1),
            nuts_id = unique(nuts3$NUTS_ID)) %>% 
            dplyr::left_join(visualize_impacts_across_years, 
                             by = c("nuts_id" = "nuts_id",
                                    "year_date" = "year_date")) %>% 
  mutate(MIS = ifelse(is.na(MIS), 0, MIS)) %>% 
  sf::st_as_sf() %>% 
...
#plot(hc5, cex = 0.6, labels=FALSE, main = "Dendogram hierarchical clustering \n of clustering on PC1-PC3")
...
# methods to assess cluster quality -->  ward is best!!!
...
# Creating DIP with our without PCA

Only use this script when necessary

```{r, eval=FALSE}
#cluster without PCA
 clustering_no_pca <- total_annual_impact_data_transformed %>% 
  ungroup() %>% 
    dplyr::select(agriculture:fire)
...
# Dissimilarity matrix
d <- dist(clustering_no_pca, method = "euclidean")
...
# Cut tree into 4 groups
sub_grp <- cutree(hc5, k = 4)
data_clustering_no_pca <- data_clustering %>%
  dplyr::select(!cluster) %>% 
  dplyr::bind_cols(sub_grp) %>% 
   dplyr::rename(cluster = ...14) %>% 
  mutate(cluster = as.factor(cluster)) %>% 
    dplyr::bind_cols(total_annual_impact_data_transformed) 



comparison_df <- data_clustering_no_pca %>% 
  dplyr::select(cluster) %>% 
  dplyr::rename(cluster_no_pca = cluster) %>% 
  dplyr::bind_cols(data_clustering)


equal_rows <- sum(comparison_df$cluster == comparison_df$cluster_no_pca)

print(equal_rows/nrow(equal_rows))
...
table <- number_features %>% 
  ...
  gt_add_divider(columns = "ggplot_spatial", color = "grey") %>% 
   tab_footnote(
    footnote = "Map indicates areas with higher prevelance of selected cluster",
    locations = cells_column_labels(
      columns = ggplot_spatial
    )
  ) %>% 
   tab_footnote(
    footnote = "Graph illustrates prevalence of cluster over time",
    locations = cells_column_labels(
      columns = ggplot
    )
  ) %>% 
   tab_footnote(
    footnote = "Graph illustrates prevalence different impact classes compared to
    other clusters",
    locations = cells_column_labels(
...
transitions_plot <- ggplot(as.data.frame(data_alluvial),
  ...
  geom_flow(color = "grey50", fill = "white", alpha = .5) +
...
dendogram_years <- hclust_years_data %>% 
  dist() %>% 
  hclust() %>% 
 ggdendrogram(., rotate = FALSE, size = 4)

#these things need to be assemled in coreldraw
ggsave(plot = dendogram_years, filename = "hierarchical_plot/hierarchical_clustering_years.jpg",
...
ggsave(plot = plot_2003, filename = "hierarchical_plot/plot_2003.jpg",
...
ggsave(plot = plot_2015, filename = "hierarchical_plot/plot_2015.jpg",
       units = "cm", width = 3.3, height = 6)
...
ggsave(plot = plot_2018, filename = "hierarchical_plot/plot_2018.jpg",
       units = "cm", width = 3.3, height = 6)
...
ggsave(plot = plot_2019, filename = "hierarchical_plot/plot_2019.jpg",
       units = "cm", width = 3.3, height = 6)
...
ggsave(plot = plot_2020, filename = "hierarchical_plot/plot_2020.jpg",
       units = "cm", width = 3.3, height = 6)
...
ggsave(plot = plot_2022, filename = "hierarchical_plot/plot_2022.jpg",
       units = "cm", width = 3.3, height = 6)
...
#this figure seems so wrong, look at 1-->1 and 2--> 2 being te same 


```


### Computing variable ranking

```{r}
library(varrank)

varrank_computations_binary_aggregated <- function(cluster_selected){

varrank_input <- data_clustering %>% 
  dplyr::mutate(cluster_1 = ifelse(cluster == cluster_selected, 1, 0)) %>% 
...
glm(cluster_1 ~ ., data = varrank_input, family = binomial)


}

model_list <- list(varrank_computations_binary_aggregated(1),
                     varrank_computations_binary_aggregated(2),
                     varrank_computations_binary_aggregated(3),
                     varrank_computations_binary_aggregated(4))

stargazer::stargazer(model_list, title = "Logistic Regression Models", type = "latex", font.size = "tiny")

```

```{r}

varrank_sequence_id_computation <- function(sequence_selected){
...
varrank_input <- varrank_sequences_data %>% 
...
glm(sequence_id ~ ., data = varrank_input, family = binomial)
...
model_list <- list(varrank_sequence_id_computation(1),
                  varrank_sequence_id_computation(2),
                  varrank_sequence_id_computation(3),
                  varrank_sequence_id_computation(4),
                  varrank_sequence_id_computation(5))




stargazer::stargazer(model_list, title = "Logistic Regression Models", type = "text", font.size = "tiny")
...
Measure relationships using Mann-Whitney U test on DIP
```{r}
