...
ggsave(plot = last_plot(),
       device = "eps",
       filename = "data_descriptive_figure.eps",
       width = 70,
       height = 27,
       units = "cm")
...
### Technical table summarising DIP for supplementary material
 for each variable, the minimum height corresponds to the minimum of that variable over the whole dataset, the maximum height to the maximum of the variable on the whole dataset.
...
```{r}
data_clustering %>% 
  dplyr::select(year_date, nuts_id, cluster) %>% 
  dplyr::filter(year_date %in% c(2018,2019,2020,2022)) %>% 
  pull(nuts_id) %>% 
  unique(.) %>% 
  length(.)
...
names(df_heatmap) <- c("Variable1", "Variable2", "Similarity")
 ...
 ggplot(df_heatmap, aes(Variable1, Variable2, fill = Similarity, label = Similarity)) +
...
```{r}
data_alluvial_test <- data_alluvial
colnames(data_alluvial_test) <- c("y_2018", "y_2019", "y_2020", "y_2022", "n")

data_alluvial_test %>% 
  dplyr::filter(y_2018 == 2) %>% 
  dplyr::filter(y_2019 == 3) %>% 
  summarise(nuts = sum(n))
```
...
expand_grid(nuts_id = unique(data_clustering$nuts_id), 
            year_date = as_factor(c(2018,2019,2020,2022))) %>% 
  dplyr::left_join(tmp, by = c("nuts_id" = "nuts_id", "year_date" = "year_date")) %>% 
  dplyr::mutate(cluster = ifelse(is.na(cluster),5,cluster)) %>% 
  dplyr::mutate(prior_cluster = lag(cluster)) %>% 
  dplyr::select(cluster, prior_cluster) %>% 
  drop_na(prior_cluster) %>% 
  mutate(identical = ifelse(cluster == prior_cluster, "Identical", "Not Identical")) %>%
  count(identical)
...
```{r}
data_clustering %>% 
  mutate(cluster  = ifelse(cluster == 3,1,0)) %>% 
  dplyr::group_by(nuts_id) %>% 
  summarise(cluster_present = sum(cluster, na.rm = TRUE)) %>% 
  ungroup() %>% 
  group_by(cluster_present) %>% 
  count()
```
...
for (index in seq_along(continuous_var_names)) {
  ...
  zeros <- tibble(binary = binary,
         continuous = continuous) %>% 
    dplyr::filter(binary == 0) %>% 
    pull(continuous)
  
   ones <- tibble(binary = binary,
         continuous = continuous) %>% 
    dplyr::filter(binary == 1) %>% 
    pull(continuous)
  
  result <- wilcox.test(x = ones, y = zeros, alternative = "greater")
