...
Current situation: 
* We have no clear cluster structures, it's rather a point cloud and we're forcing some clusters in to simplify our analysis. Can we do this, and how do we reason this? 
--> 'clustering serves as a means to achieve this goal. The clusters identified may represent different types of drought impacts or response patterns, even if they are not perfectly distinct.'...'the silhouette coefficient is low, the identified clusters could still be meaningful and informative based on expert knowledge.'...' Even if the clusters are not well-separated, they might still provide insights into the data, and the patterns identified could be relevant for understanding drought impacts.'...'still being transparent about this issue'
...
s=NULL
for(i in 2:10){
   k=cutree(hclust(dist(data_clustering),method="ward.D"),i)
   s[i]=summary(silhouette(k,dist(data_clustering)))$si.summary[3]
}
s
```
...
pca_result$x %>% 
  as_tibble() %>% 
  dplyr::bind_cols(as_tibble(sub_grp)) %>% 
ggplot(aes(x = PC1, y = PC2, color = as_factor(value)))+
  geom_point()
```
